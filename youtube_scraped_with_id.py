...
API_KEYS = [os.getenv(f'YOUTUBE_API_KEY_{i}') for i in range(1, 10)]
...
class YouTubeScraper:
    ...
    # --- changed: we no longer resolve handles, we accept IDs directly ---
    ...
    def save_checkpoint(self, processed_rows, last_channel_id):
        checkpoint = {
    ...
    def process_channel(self, channel_id, row_index):
        ...
        records = []
        try:
            # directly use the given channel_id
            if not channel_id:
                logger.warning(f"No channel ID for row {row_index}")
                return True
            ...
            logger.info(f"✅ {channel_id} ({len(videos)} videos)")
            return True

        except HttpError as e:
            msg = str(e).lower()
            if 'quotaexceeded' in msg or 'dailylimitexceeded' in msg:
                logger.error(f"🌡️ RATE LIMIT EXHAUSTED while processing {channel_id}")
                return False
    ...
    def run(self):
        ...
        # expect a channel_id column; if not present, try to infer from something that looks like UC...
        if 'channel_id' not in df.columns:
            for col in df.columns:
                if df[col].astype(str).str.startswith('UC').any():
                    df.rename(columns={col: 'channel_id'}, inplace=True)
                    break
            else:
                logger.error("❌ No channel_id column found (must contain YouTube channel IDs starting with 'UC')")
                return
        ...
        for i in range(start_row, total):
            channel_id = str(df.iloc[i]['channel_id']).strip()
            logger.info(f"[{i+1}/{total}] {channel_id}")

            success = self.process_channel(channel_id, i)
            if success:
                batch_counter += 1
                self.save_checkpoint(i + 1, channel_id)
